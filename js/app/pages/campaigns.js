export const campaigns = {
  ...
  methods: {
    ...
    // ИСПРАВЛЕННЫЙ МЕТОД LINE
    line: function (item) {
      // Используем nextTick, чтобы убедиться, что попап открылся и элемент #chartOuter существует в DOM
      this.$nextTick(() => {
    ...
    // ИСПРАВЛЕННЫЙ МЕТОД RENDERCHART
    renderChart: function (item) {
      ...
      setTimeout(function () {
        ...
        // --- ОТЛАДКА: Посмотрим, что реально лежит в line ---
        console.log('Данные Line от сервера:', item ? item.line : 'Нет данных')
        // ---------------------------------------------------
        ...
        while (current <= end) {
          ...
          // Формируем ДВА варианта ключа, чтобы наверняка найти данные
          let keyDateDot = `${day}.${month}.${year}` // 01.01.2025
          let keyDateDash = `${year}-${month}-${day}` // 2025-01-01

          // Лейбл для оси X (всегда красивый)
          dates.push(`${day}.${month}.${year}`)

          // Ищем данные по любому из ключей
          let stats = null
